...
vd_t StationaryGroupedEvo(const std::vector<std::vector<double>>& p_fix, const std::vector<double>& self_coop_levels, double benefit, double sigma_out, double r_mut, size_t T_max, double dt) {
  ...
  // measure elapsed time
  {
    ...
    for (size_t n = 0; n < N_max; n++) {
      x = SolveByRungeKutta(calc_x_dot, x, dt, n_iter);
      if (n % n_interval == 0) {
        fout << n*dt*n_iter << ' ';
        double pc = 0.0;
